...
// [급구] 웹의 /urgent 페이지: 구매글과 분리된 급구 전용 목록
export default function MarketScreen() {
  ...
  const filtered = useMemo(() => {
    ...
    return posts.filter((post) => {
      if (post.type !== "urgent" || (category !== "전체" && post.category !== category)) return false;
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: palette.paper }} edges={["top"]}>
      ...
      <FlatList
        ...
        ListHeaderComponent={
          <View>
            <PostSearchBar query={query} onChangeQuery={setQuery} placeholder="급구 제목, 지역, 카테고리를 검색해요" resultCount={filtered.length} />
