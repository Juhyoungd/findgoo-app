...
export function PostCard({ post, saved, totalOfferCount, onOpen, onToggleSaved }: PostCardProps) {
  ...
  return (
    <Pressable onPress={() => onOpen(post)} style={[styles.card, { backgroundColor: palette.white, borderColor: palette.line }, isUrgent && { borderLeftColor: palette.orange, borderLeftWidth: 3 }, !isOpen && styles.notOpen]}>
      <View style={styles.head}>
        <View style={styles.badgeRow}>
          ...
          <Text style={[styles.badge, { backgroundColor: post.status === "reserved" ? `${palette.orange}22` : post.status === "closed" ? `${palette.muted}22` : palette.paper, color: post.status === "reserved" ? palette.orange : palette.muted, fontWeight: post.status === "open" ? "700" : "900" }]}>{statusLabel[post.status]}</Text>
        </View>
